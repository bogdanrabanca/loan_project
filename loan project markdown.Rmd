...
```{r}
loan_data %>% group_by(IncomeRange) %>% summarize(avg_loan=mean(LoanOriginalAmount)) %>% arrange(desc(avg_loan)) %>% ggplot()+geom_bar(aes(x=reorder(IncomeRange,avg_loan),y=avg_loan),stat='identity')+coord_flip()
```
...
```{r}
ggplot(loan_data)+geom_freqpoly(aes(x=LoanOriginalAmount),size=1,bins=10)+facet_wrap(~IncomeRange,nrow=4)
```
...
```{r}
loan_data %>%  filter(IncomeRange!='$0') %>% ggplot()+geom_boxplot(aes(x=IncomeRange,y=DebtToIncomeRatio))+coord_flip()
```
...
#*Available bank credit and revolving credit percentage versus default*(MAYBE TAKE THIS OUT)

Next I try to see if there is a relationship between available bank credit, the percentage of credit used, and default:

```{r}
loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% ggplot(aes(x=AvailableBankcardCredit,y=RevolvingCreditBalance,color=default))+geom_point(alpha=1/3)+coord_cartesian(xlim=c(0,75000),ylim=c(0,75000))
```

This chart is too crowded, so I switch to a histogram plot and look at the relationships separately:
```{r}
loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% ggplot()+geom_histogram(aes(x=AvailableBankcardCredit,fill=default),binwidth=1000,alpha=.2)+coord_cartesian(xlim=c(0,20000))
...
plot1 <- loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% filter(default==0) %>% ggplot(aes(x=AvailableBankcardCredit))+geom_histogram(aes(y=stat(count/sum(count))),binwidth=200,alpha=.7,fill='Blue')+coord_cartesian(xlim=c(0,20000),ylim=c(0,.2))

plot2 <- loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% filter(default==1) %>% ggplot(aes(x=AvailableBankcardCredit))+geom_histogram(aes(y=stat(count/sum(count))),binwidth=200,alpha=.7,fill='Red')+coord_cartesian(xlim=c(0,20000),ylim=c(0,.2))
...
```{r}
plot1 <- loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% filter(default==0) %>% ggplot(aes(x=AvailableBankcardCredit))+geom_histogram(aes(y=stat(count/sum(count))),binwidth=200,alpha=.7,fill='Blue')+coord_cartesian(xlim=c(20000,60000),ylim=c(0,.01))

plot2 <- loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% filter(default==1) %>% ggplot(aes(x=AvailableBankcardCredit))+geom_histogram(aes(y=stat(count/sum(count))),binwidth=200,alpha=.7,fill='Red')+coord_cartesian(xlim=c(20000,60000),ylim=c(0,.01))
...
```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization))+coord_cartesian(x=c(2,8),y=c(0,50))
...
```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization),binwidth=.1)+coord_cartesian(x=c(0,2))

```

```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization),binwidth=.01)+coord_cartesian(x=c(0,2))
```

Next I break down the histogram based on income range: 

```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization),binwidth=.01)+coord_cartesian(x=c(0,1.25))+facet_wrap(~IncomeRange,nrow=4)
```
...
```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization,y=stat(count/sum(count))),binwidth=.01)+coord_cartesian(x=c(0,1.25))+facet_wrap(~IncomeRange,nrow=4)
```

One explanation might be that higher earners just make more money so they can afford to run a higer bill, but since they are also likely to have a higher credit limit, this explanation is not entirely satisfactory. Perhaps here too, just as before, the expplanation is that for low-income earners, having a low revolving balance is almost necessary for getting any kind of loan approved by the bank, while higher income owners are not rejected simply because they have a high credit card bill. Interestingly, if we further distinguish between homeowners and renters, it looks like the higher your revolving balance, the more likely you are to own your home (at all income levels):

```{r}
ggplot(loan_data)+geom_histogram(aes(x=BankcardUtilization,y=stat(count/sum(count)),fill=IsBorrowerHomeowner),binwidth=.01)+coord_cartesian(x=c(0,1.25))+facet_wrap(~IncomeRange,nrow=4)
```
