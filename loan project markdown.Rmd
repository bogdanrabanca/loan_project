...
This chart is too crowded, so I switch to a density plot and look at the relationships separately:
```{r}
loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% ggplot()+geom_density(aes(x=AvailableBankcardCredit,fill=default),alpha=.2)+coord_cartesian(xlim=c(0,20000))

```

A higher proportion of defaults happened when bororowers had less bank credit. 

```{r}
loan_data %>% mutate (default=as.character(ifelse(LoanStatus=='Defaulted',1,0))) %>% ggplot()+geom_density(aes(x=RevolvingCreditBalance,fill=default),alpha=.2)+coord_cartesian(xlim=c(0,60000))
```

*It's only when revolving balance is really high (around \$45,000) where a higher percentage of these mortgages default. What is going on with this chart? one of the density plots seems bigger than the other even though both should have the same area. Maybe this is not a good plot to use.*
